...
const StatCard = ({ label, value, icon: Icon, color, bg }) => (
  ...
  onMouseLeave={(e) => {
    ...
    <div>
      <p style={{ fontSize: '11px', fontWeight: '600', color: '#94A3B8', margin: 0, textTransform: 'uppercase', letterSpacing: '0.6px' }}>{label}</p>
      <p style={{ fontSize: '26px', fontWeight: '800', color: '#1E293B', margin: '4px 0 0 0', letterSpacing: '-0.5px' }}>{value}</p>
    </div>
  </div>
);
...
const Dashboard = () => {
  const { players, coaches, sessionHistory, fetchCoaches, fetchPlayers } = useStore();
  ...
  const topPerformers = [...players].sort((a, b) => b.totalPoints - a.totalPoints).slice(0, 5);
      ...
      <Layout>
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '24px 28px' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '32px' }}>
            {[1,2,3,4].map(i => (
              <div key={i} style={{ background: '#fff', borderRadius: '16px', padding: '20px', border: '1px solid #EEF2F7' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: '14px', marginTop: '16px' }}>
            {[1,2,3,4,5,6].map(i => (
              <div key={i} style={{ background: '#fff', borderRadius: '14px', padding: '20px', border: '1px solid #EEF2F7', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
    ...
    <Layout>
      <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '24px 28px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px', marginBottom: '36px' }}>
          <StatCard label="Total Players" value={totalPlayers} icon={Users} color="#6366F1" bg="linear-gradient(135deg, #EEF2FF, #E0E7FF)" />
          <StatCard label="Total Coaches" value={totalCoaches} icon={Award} color="#10B981" bg="linear-gradient(135deg, #ECFDF5, #D1FAE5)" />
          <StatCard label="Total Sessions" value={totalSessions} icon={BookOpen} color="#F59E0B" bg="linear-gradient(135deg, #FFFBEB, #FEF3C7)" />
          <StatCard label="Completion Rate" value={`${adminCompletionRate}%`} icon={Target} color="#EC4899" bg="linear-gradient(135deg, #FDF2F8, #FCE7F3)" />
        </div>

        <div style={{ marginBottom: '40px' }}>
          <h2 style={{ fontSize: '17px', fontWeight: '700', color: '#1E293B', margin: '0 0 18px 0', letterSpacing: '-0.2px' }}>Management Center</h2>
        ...
        {topPerformers.length > 0 && (
          <div style={{ marginBottom: '40px' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '18px' }}>
              <h2 style={{ fontSize: '17px', fontWeight: '700', color: '#1E293B', margin: 0, letterSpacing: '-0.2px' }}>Top Performers</h2>
              ...
              {topPerformers.map((player, idx) => {
                ...
                return (
                  <div key={player.playerId} style={{
                    ...
                    border: idx === 0 ? `2px solid ${medal.ring}60` : '1px solid #EEF2F7',
                  ...
                  onMouseLeave={(e) => {
                    ...
                    e.currentTarget.style.borderColor = idx === 0 ? `${medal.ring}60` : '#EEF2F7';
                  }}>
                    <div style={{
                    ...
                    <p style={{ fontSize: '14px', fontWeight: '700', color: '#0F172A', margin: '0 0 4px 0', paddingRight: '40px' }}>
                      {player.name}
                    </p>
                    <p style={{ fontSize: '12px', color: '#94A3B8', margin: '0 0 16px 0', fontWeight: '500' }}>
